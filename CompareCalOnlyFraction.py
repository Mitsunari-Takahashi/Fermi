...
def main(datafile, suffix):
    ...
    hRaw_CalTwrEdgeCntr = ROOT.TH2F('hRaw_CalTwrEdgeCntr', 'Raw number of events;CalTwrEdgeCntr;Cal1MomZDir', NBIN_CTE, LOWEDGE_CTE, UPEDGE_CTE, NBIN_CTH, LOWEDGE_CTH, UPEDGE_CTH)
    hCalOnly_CalTwrEdgeCntr = ROOT.TH2F('hCalOnly_CalTwrEdgeCntr', 'Number of CalOnly events;CalTwrEdgeCntr;Cal1MomZDir', NBIN_CTE, LOWEDGE_CTE, UPEDGE_CTE, NBIN_CTH, LOWEDGE_CTH, UPEDGE_CTH)
...
#    hCalOnly_CalLATEdge = ROOT.TH2F('hCalOnly_CalLATEdge', 'Number of CalOnly events;CalLATEdge;Cal1MomZDir', NBIN_CTE, LOWEDGE_CTE, UPEDGE_CTE, NBIN_CTH, LOWEDGE_CTH, UPEDGE_CTH)
    ...
    for iEvt in range(NEVT):
        ...
        if ch.FT1CalZenithTheta<ZENITH_CUT and ch.Cal1RawEnergySum>=20000 and ch.FswGamState==0:
            ...
            hRaw_CalTwrEdgeCntr.Fill(ch.CalTwrEdgeCntr, ch.Cal1MomZDir)
            ...
            if  (ch.TkrNumTracks==0 or (math.log10(max(ch.CalTrackAngle,1E-4)) > (0.529795)*(ch.EvtJointLogEnergy < 3.000000)  + ((1.0)*((0.529795)*(1.0)+(-1.379791)*(pow((ch.EvtJointLogEnergy-3.000000)/0.916667,1))+(0.583401)*(pow((ch.EvtJointLogEnergy-3.000000)/0.916667,2))+(-0.075555)*(pow((ch.EvtJointLogEnergy-3.000000)/0.916667,3))))*(ch.EvtJointLogEnergy >= 3.000000 and ch.EvtJointLogEnergy <= 5.750000) + (-0.398962)*(ch.EvtJointLogEnergy >  5.750000)) ):
                ...
                hCalOnly_CalTwrEdgeCntr.Fill(ch.CalTwrEdgeCntr, ch.Cal1MomZDir)
    ...
    hRaw_CalTwrEdgeCntr.Write()
    hCalOnly_CalTwrEdgeCntr.Write()
    hFraction_CalTwrEdgeCntr = hCalOnly_CalTwrEdgeCntr.Clone("hFraction_CalTwrEdgeCntr")
    hFraction_CalTwrEdgeCntr.SetTitle("Fraction of CalOnly events")
    hFraction_CalTwrEdgeCntr.Divide(hRaw_CalTwrEdgeCntr)
    hFraction_CalTwrEdgeCntr.Write()
