...
def likelihood_grb_analysis(name, mode, emin, emax, eref, roi, spectraltype, refit, force, suffix, grbcatalogue, modelonly, sptmin, sptmax, outdir, binned, masifps, scalefactor=1., gti_external=None):
    ...
    if nrough<1:
        ana.order_likeratio(nseries=1)
    elif nrough<5:
        ana.order_likeratio(nseries=0)
...
def main(namemin, namemax, mode, emin, emax, eref, roi, spectraltype, refit, force, suffix, grbcatalogue, modelonly, sptmin, sptmax, outdir, binned, masifps, quanta, bsub):
    ...
    if bsub==False:
        ...
        likelihood_grb_analysis(namemin, mode, emin, emax, eref, roi, spectraltype, refit, force, suffix, grbcatalogue, modelonly, sptmin, sptmax, outdir, binned, masifps) #, scalefactor=scalefactors[idx][pha])
    else:
        ...
        for (irow , row) in enumerate(tb):
            ...
            if not os.path.exists(name):
                os.mkdir(name)
            acmd = ['bsub', '-o','{0}/{1}/GRB{1}_{2}{3}.log'.format(outdir, name, mode, suffix if suffix=='' else '_'+suffix), '-J','{0}{1}'.format(name[:6], mode[:2]), '-W','300', 'python', '/u/gl/mtakahas/work/PythonModuleMine/Fermi/STLikelihoodAnalysis/STLikelihoodGRBAnalysis.py', '-m', mode, '--emin', str(emin), '--emax', str(emax), '--eref', str(eref), '-s', suffix, '--roi', str(roi), '--spectraltype', spectraltype, '--sptmin', str(sptmin), '--sptmax', str(sptmax), '--outdir', '{0}/{1}'.format(outdir, name), '--namemin', name, '--quanta', quanta]
            if force==True:
                acmd.append('--force')
            if refit==True:
                acmd.append('--refit')
