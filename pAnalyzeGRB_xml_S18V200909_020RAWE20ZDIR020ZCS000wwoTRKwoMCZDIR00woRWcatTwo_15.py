...
#S16V200909_020RAWE20ZDIR010ZCS000wwoTRKwoMCZDIR00woRWcatTwoZDIR050_15/S16V200909_020RAWE20ZCS000wwoTRKwoMCZDIR00woRW_15_S11D_catTwoZDIR050Log_roc.root"] #par[2]
nameVarBDT = "S18V200909_020RAWE20ZDIR020ZCS000wwoTRKwoMCZDIR00woRWcatTwo_15_catZDIR060_BDTG500D06_catZDIR060"
nameFileSuffix = nameVarBDT
aCutMVA = []
for iRocFile in range(len(nameFileRoc)):
    aCutMVA.append(CutBDT(nameFileRoc[iRocFile], aCutEGB))
aCosZ=[0.2, 0.6, 1.0]
cutMVA_comb = CutBDT_2D(aCutMVA, aCosZ)
print cutMVA_comb
...
#    for iRegio in range(1+nOff):
    ...
    for iEvent in range(nEvent):
        ...
        p[0] = (chainData.S18V200909_020RAWE20ZDIR020ZCS000wwoTRKwoMCZDIR00woRWcatTwo_15_catZDIR060_BDTG500D06_catZDIR060+1.0)/2.0
...
#        binEnergy = max(min(nEnergyBin-1, int((e[0]-vEnergyLow)/vEnergyBinWidth * (int(e[0]<vEnergyLow)*(-2)+1)) ), 0)
        if (chainData.TkrNumTracks>0) and (math.log10(max(chainData.CalTrackAngle,1E-4)) <= (0.529795)*(e[0] < 3.000000) + ((1.0)*((0.529795)*(1.0)+(-1.379791)*(pow((e[0]-3.000000)/0.916667,1))+(0.583401)*(pow((e[0]-3.000000)/0.916667,2))+(-0.075555)*(pow((e[0]-3.000000)/0.916667,3))))*(e[0] >= 3.000000 and e[0] <= 5.750000) + (-0.398962)*(e[0] > 5.750000)) and chainData.EvtCalCsIRLn>4 and chainData.WP8CTPSFTail>0.05 and chainData.WP8CTBestEnergyProb>0.1 and chainData.FswGamState == 0: # CalTkr
            ...
            if -math.log10(1.0-p[0])>=cutMVA_comb.aHtgCutBDT[0].GetBinContent(cutMVA_comb.aHtgCutBDT[0].GetXaxis().FindBin(e[0]), cutMVA_comb.aHtgCutBDT[0].GetYaxis().FindBin(cth[0])): #CalOnly_R100
                if -math.log10(1.0-p[0])>cutMVA_comb.aHtgCutBDT[1].GetBinContent(cutMVA_comb.aHtgCutBDT[1].GetXaxis().FindBin(e[0]), cutMVA_comb.aHtgCutBDT[1].GetYaxis().FindBin(cth[0])): #CalOnly_R30
                    if -math.log10(1.0-p[0])>cutMVA_comb.aHtgCutBDT[2].GetBinContent(cutMVA_comb.aHtgCutBDT[2].GetXaxis().FindBin(e[0]), cutMVA_comb.aHtgCutBDT[2].GetYaxis().FindBin(cth[0])): #CalOnly_R10
                        s[0]=16384#3
