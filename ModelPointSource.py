        ...
        for iEne in range(1, TP_HTG_KING[0].GetNbinsX()+1):
            ...
            for iTh in range(1, HTG1_LT.GetNbinsX()+1):
                for ipar in range(3): # Setting the parameters of King function
                    # PSF
                    fc_King_annulus.FixParameter(ipar, TP_HTG_KING[ipar].GetBinContent(iEne, TP_HTG_KING[ipar].GetYaxis().FindBin(HTG1_LT.GetBinCenter(iTh))))
                ...
                scale_aeff = HTG2_ACCEPTANCE.GetBinContent(HTG2_ACCEPTANCE.GetXaxis().FindBin(TP_HTG_KING[0].GetXaxis().GetBinCenter(iEne), HTG2_ACCEPTANCE.GetYaxis().FindBin(HTG1_LT.GetXaxis().GetBinCenter(iTh)))) / (2*math.pi*HTG2_ACCEPTANCE.GetYaxis().GetBinWidth(iY)) # Effective area
                scale_exp = scale_aeff * HTG1_LT.GetBinContent(iTh) # Integrated exposure value for a certain energy and inclination angle
                for ipix in range(NPIX):
                    ...
                    if deg_pix2src < THRESHOLD_ANGDIST:
                        rad_pix2src = float(ang_pix2src.to_string(unit=u.rad, decimal=True)) # Angular distance between the source and each pixel in radians
                        scale_psf = fc_King.Eval(rad_pix2src)*hppf.nside2pixarea(NHPSIDE)
                        htg2_model.Fill(ipix, htg2_model.GetBinCenter(iEne), flux_true_diff*scale_psf*scale_exp*factor_norm)
...
def main(name, sed, ra, dec, king, acceptance, livetime):
    ...
    HTG_LT = FILR_LT.Get('htgLt_GRB160509374_0_yx') #TBD
