...
class Healcube:
    ...
    def draw(self, mollweide=False):
        for ienr in range(self.eregion.nBin+1):
            ...
            plt.savefig("png/{0}_{1}_NSIDE{2}_E{3}-{4}.png".format(self.name, self.evtclass, self.NSIDE, int(100*self.eregion.getBin(enrlow)[0]+0.5), int(100*self.eregion.getBin(enrup)[1]+0.5)))
            plt.close()
