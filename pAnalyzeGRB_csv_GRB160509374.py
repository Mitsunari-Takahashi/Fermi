...
for nameFileIn in listFileIn:
    ...
    nameGrb = "160509374" #nameFileIn[indexGrbName:indexGrbNameEnd]
    for iGrb in range(num_lines-1):
        if int(nameGrb) == int(csv.ix[iGrb,'name']): #grb.findtext("./GRBNAME")==nameGrb: 
...
#    for iRegio in range(1+nOff):
    ...
    for iEvent in range(nEvent):
        ...
        aDist = []
        if vSepCut == 68:
            distCut = htgPerf.getPSF68_cth(c[0]-1, 0, 5.063819408, 0.985249156718) + err_rad
        elif vSepCut == 95:
            distCut = htgPerf.getPSF95_cth(c[0]-1, 0, 5.063819408, 0.985249156718) + err_rad
        else:
            distCut = htgPerf.getPSF95_cth(c[0]-1, 0, 5.063819408, 0.985249156718) + err_rad
            print "Cut value on the angular separation:", vSepCut, "is wrong."
#        distCut = htgPerf.getPSF68_cth(c[0]-1, 0, 5.063819408, 0.985249156718) + err_rad
        for iRegio in range(1+nOff):
        ...
        #     sys.stdout.flush()
    cEvent.SetTitle("GRB {0} gamma-like events".format(nameGrb))
    cEvent.cd()
    mgr.Draw("AP")
