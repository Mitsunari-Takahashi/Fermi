...
class SourceObject:
    def __init__(self, name, met0, ra, dec, tmin=0, tmax=100000, emin=10000, emax=316228, deg_roi=1., zmax=100., suffix='', tprompt_start=0., redshift=0):
    ...
    def plot(self, ax, nplot=0):
        lst_markers = ['s', 'o', '*', 'D', 'x']
        nmarker = nplot / NMARKER_STYLE
        ax.scatter(self.times, self.energies/1000., alpha=0.75, label=self.config['name'], marker=lst_markers[nmarker])
...
def main(emin, emax, tmin, tmax, roi, redshift, suffix, pathout, figform, longonly, exceptions):
    fig = plt.figure(figsize=(15,4.5))
    ax = fig.add_axes((0.07, 0.15, 0.9, 0.75))
    ax.set_title('Events within {rad:2.1f} degrees from LAT-GRBs'.format(rad=roi))
    ...
    lst_lat = ReadLATCatalogueInfo.select_small_error(lst_lat, 0.3)
    ngrb_plotted = 0
    for (irow , tb_one) in enumerate(lst_lat):
        ...
        if redshift==False or z>0:
            src = SourceObject(name=name, met0=trigger_met, ra=tb_one['RA'], dec=tb_one['DEC'], tmin=tmin, tmax=tmax, emin=emin, emax=emax, deg_roi=roi, tprompt_start=t05, redshift=z if redshift==True else 0)
